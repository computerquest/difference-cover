...
int main(int argc, char *argv[]) {
	...
	if (id == 0) {
		MPI_Irecv(&found_item, p, MPI_INT, MPI_ANY_SOURCE, found_tag, MPI_COMM_WORLD, &found_request);
	}
	else {
		MPI_Ibcast(&found_item, 1, MPI_INT, 0, MPI_COMM_WORLD, &found_request);
	}
	...
	if (id == 0) {
		...
		while (cont) {
			MPI_Recv(&differenceCover, p, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Recv(&dsize, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

			if (coverOfSize(p, dsize, differenceCover, testCover)) {
				MPI_Send(&differenceCover, p, MPI_INT, 0, found_tag, MPI_COMM_WORLD);
			}
...
void cover(const int p, ofstream &out) {
	...
	for (int x = min; x <= max; x++) {
		//send the stuff for the slaves form here [look at the gray code stuf]
