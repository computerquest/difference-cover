...
int isCover(const int p, const int *differenceCover, int *testCover)
{                                       //difference cover and test cover are binary, p is n,
                                        // Takes half the time, unless the double pointer casting takes too long. (Check this.)
    double *temp = (double *)testCover; //what is the purpose of having this
    for (int x = 0; x < p / 2; x++)
        temp[x] = 0;
...
{ //counts number of 1s (number of elements)
    int f = 0;
    for (int x = 0; x < p; x++)
        f += cover[x];
    return f;
